...
another same-user program from invoking the bridge. macOS, its public framework
...
- The user treats a target grant as authorization for the selected harness to
  exercise its approved capabilities until Stop, disconnect, expiry, target
  recreation, or permission loss revokes that grant.
...
| An MCP client calls tools without consent | A first explicit native decision or an exact versioned policy bound to the verified requester, signed target, and capability ceiling; one safe AX-bound window; mandatory indicator; no ambient default target | A compromised already-approved harness can operate within its active grant until Stop, disconnect, target recreation, or idle expiry; do not grant sensitive targets to an untrusted harness |
| Client changes an intended action | Current frame and semantic-target binding; strict schemas; target/capability checks immediately before dispatch; visible rail and Stop | Grant-scoped authorization deliberately permits the approved harness to choose actions within the approved capabilities; it does not provide transaction-level human review |
...
| Secure/password content is read, selected, or changed outside an approved grant | Secure Accessibility fields stay redacted and unselectable; direct secure-text value writes require an exact current element bound to the approved grant and frame; protected content, secure descendants, and ambiguous ancestry stay denied; no clipboard read | Pixel capture may still show secrets the app itself renders visibly; write-only input can reach an exact secure field during an approved grant without a second prompt |
...
actions are denied, and `computer_set_value` can reach it only when the exact
field is bound to the current grant and frame. The host does not downgrade to a private
...
3. treat granting a sensitive target as authorization for the harness to act
   within the approved capability set until the grant is revoked;
...
The native host enforces grants and hard blocks, but it cannot decide
